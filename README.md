...
every reader. Lookups are progressive and complementary: Pali words first
show a near-instant entry from the
[Digital Pāḷi Dictionary](https://www.dpdict.net) (CC BY-NC-SA 4.0) with the
authoritative meanings and grammar, which the LLM — given the DPD entry as
context — then extends in place with the morpheme breakdown, etymology, and
cognates.
...
but new entries are capped per day. In production builds the add form and
delete buttons are hidden from visitors regardless of server config — as
the owner, open `/#add` once and enter the passphrase; that browser then
always shows the owner UI. Dev builds always show it.
